...
param (
    ...
    [Parameter(
            Mandatory = $false,
            HelpMessage = "Prefix is a lowercase, alphanumeric name, starting with a letter. '_' and '-' are allowed.")]
...
$TerrafomMinimumVersion = [version]"0.13.2"
...
if ($UtilResourceGroupName -eq "") {
    $UtilResourceGroupName = "$($Prefix.Replace('-', '_'))_$($EnvironmentName)_tfstate_rg".ToLower()
}
...
Write-Verbose "TF_VAR_environment_name         $EnvironmentName"
...
function CreateOrUpdateTerraformBackend {
    ...
    $StorageAccountNamePrefix = $Prefix.Replace("_", "").Replace("-", "").SubString(0, [System.Math]::Min(10, $Prefix.Replace("_", "").Replace("-", "").Length))
